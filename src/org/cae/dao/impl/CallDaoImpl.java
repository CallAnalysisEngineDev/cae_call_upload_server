...
public class CallDaoImpl implements ICallDao {
	...
	@Override
	public DaoResult<CallRecord> updateCallVersionDao(
			final List<String> songIds, final CallRecord callRecord) {
		try {
			String sql = "UPDATE call_record " + "SET call_version = ? "
					+ "WHERE song_id = ?";
	...
	private void saveCallDao(final List<Song> songs, final CallRecord callRecord) {
		...
		template.batchUpdate(sql, new BatchPreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps, int count)
				...
				ps.setString(3, CALL_SOURCE_PREFIX + song.getSongName()
						+ ".html");
